...
Start by creating an environment variable with the endpoint name you want to create, and a variable pointing to an S3 location under which you want to store the data captured by the endpoint and ground truth labels. The following command will append these variables to the `.env` file and export them in your current shell. Ensure to replace `[YOUR BUCKET]` with the name of the S3 bucket where you want to store the data:

```bash
export $( (tee -a .env << EOF
ENDPOINT_NAME=penguins
DATA_S3_LOCATION=s3://[YOUR BUCKET]/penguins/
EOF
) && cat .env | xargs)
```
...
python3 pipelines/deployment.py --environment=pypi run \
    ...
    --region $AWS_REGION \
    --data-capture-destination-uri $DATA_S3_LOCATION/datastore
```
...
python3 pipelines/endpoint.py --environment=pypi run \
    --action labeling \
    --target sagemaker \
    --target-uri $DATA_S3_LOCATION/datastore \
    --ground-truth-uri $DATA_S3_LOCATION/ground-truth
```
...
```bash
python3 pipelines/monitoring.py --environment=pypi run \
    --datastore-uri $DATA_S3_LOCATION/datastore \
    --ground-truth-uri $DATA_S3_LOCATION/ground-truth
```
