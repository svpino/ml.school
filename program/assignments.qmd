...
### Chapter 5 - Custom Training Container

1. TBD: modify the docker image so we can provide a training script (instead of running the train.py script automatically)
1. TBD: configure the custom image to use debugger [Use Debugger with Custom Training Containers](https://docs.aws.amazon.com/sagemaker/latest/dg/debugger-bring-your-own-container.html)
1. TBD: create a new custom training container that uses a different library to train the model. 
1. TBD: create a livecycle policy to automatically expire old images from ecr. Check [Lifecycle policies](https://docs.aws.amazon.com/AmazonECR/latest/userguide/LifecyclePolicies.html)
1. TBD: keras 3 is awesome. try to change the backend in the train script to compare pytorch tensorflow and jax


### Chapter 6 - Tuning the Model
...
### Chapter 7 - Evaluating the Model
...
### Chapter 8 - Registering the Model
...
### Chapter 9 - Conditional Registration
...
1. TBD: Create your own container to run inference [Adapting Your Own Inference Container](https://docs.aws.amazon.com/sagemaker/latest/dg/adapt-inference-container.html)
...
### Chapter 12 - Deploying From the Pipeline
...
### Chapter 13 - Deploying From an Event

1. TBD



### Chapter 14 - Building an Inference Pipeline
...
Esto es lo ultimo que encontre:

al parecer, el train-baseline contiene el species column, por lo que esa columna se incluye en el constrains que se calcula en el baseline. luego, 
cuando esa columna no se envia al endpoint, el data monitoring job no sabe como cuadrar las columnas en orden. la solucion debe ser no incluir
la columna de species en el train-baseline.
